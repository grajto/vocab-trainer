...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(sql`
    ...
    BEGIN
      IF to_regclass('public.cards') IS NOT NULL THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_cards_owner_deck ON cards (owner_id, deck_id, id)';
      END IF;
    END $$;
  `)
  ...
  await db.execute(sql`
    DO $$
    BEGIN
      IF to_regclass('public.review_states') IS NOT NULL THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_review_states_owner_due_card ON review_states (owner_id, due_at, card_id)';
      END IF;
    END $$;
  `)
  ...
  await db.execute(sql`
    ...
    BEGIN
      IF to_regclass('public.sessions') IS NOT NULL THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_sessions_owner_started ON sessions (owner_id, started_at DESC)';
      END IF;
    END $$;
  `)
  ...
  await db.execute(sql`
    ...
    BEGIN
      IF to_regclass('public.decks') IS NOT NULL THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_decks_owner_folder ON decks (owner_id, folder_id, id)';
      END IF;
    END $$;
  `)
  ...
  await db.execute(sql`
    ...
    BEGIN
      IF to_regclass('public.test_answers') IS NOT NULL THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_test_answers_owner_test ON test_answers (owner_id, test_id, answered_at DESC)';
      END IF;
    END $$;
  `)
  ...
  await db.execute(sql`
    ...
    BEGIN
      IF to_regclass('public.payload_locked_documents_rels') IS NOT NULL THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_payload_locked_users ON payload_locked_documents_rels (users_id)';
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_payload_locked_decks ON payload_locked_documents_rels (decks_id)';
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_payload_locked_folders ON payload_locked_documents_rels (folders_id)';
      END IF;
    END $$;
  `)
}
