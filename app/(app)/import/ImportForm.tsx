...
export function ImportForm({ decks }: { decks: Deck[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-lg p-6" style={{ border: '1px solid var(--border)' }}>
      {/* Deck selector */}
      <div>
        <label className="mb-1.5 block text-xs font-medium text-[var(--text-muted)]">Docelowy zestaw</label>
      ...
      {/* CSV textarea */}
      <div>
        <label className="mb-1.5 block text-xs font-medium text-[var(--text-muted)]">Dane CSV</label>
        <textarea
          value={csvText}
        ...
        />
        <p className="mt-1.5 text-xs text-[var(--text-soft)]">Kolumny: front, back, notes (opcja), examples (opcja)</p>
      </div>
        ...
        <div className="space-y-1 rounded-lg p-4 text-sm" style={{ border: '1px solid var(--border)' }}>
          ...
          {result.errors.length > 0 && (
            <div className="mt-2 space-y-0.5">
              {result.errors.map((err, i) => <p key={i} className="text-xs text-[var(--danger)]">{err}</p>)}
            </div>
          )}
        </div>
      )}
