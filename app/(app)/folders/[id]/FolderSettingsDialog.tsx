...
export function FolderSettingsDialog({ folderId, initialName }: { folderId: string; initialName: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Nie udało się usunąć folderu')
      }
      router.push('/folders')
      router.refresh()
    } catch (err) {
          ...
          <div className="w-full max-w-md space-y-4 rounded-xl bg-white p-5" style={{ border: '1px solid var(--border)' }}>
            ...
            <div className="space-y-2 pt-2">
              ...
              <button
                ...
                disabled={saving}
                className="w-full rounded-full py-2 text-sm font-semibold transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                style={{ border: '1px solid var(--border)', color: 'var(--danger)' }}
              >
                <Trash2 size={14} />
                {saving ? 'Usuwanie…' : 'Usuń folder'}
              </button>
            </div>
          </div>
        </div>
