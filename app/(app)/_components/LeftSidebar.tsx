...
export function LeftSidebar({
  ...
  const groupTitle = 'px-2 text-xs font-semibold text-slate-500'
  const itemClass = (isActive: boolean) =>
    `flex min-h-10 items-center gap-2 rounded-xl border px-3 py-2 text-sm transition ${
      isActive
        ? 'border-indigo-100 bg-indigo-50 text-indigo-600 font-semibold'
        : 'border-transparent text-slate-700 hover:border-slate-100 hover:bg-slate-50'
    }`

  const nav = (
    <aside className="flex h-full flex-col overflow-y-auto border-r border-slate-200 bg-white px-3 py-4">
      <div className="mb-1 flex items-center justify-between px-2 lg:hidden">
        <span className="text-sm font-semibold text-slate-700">Menu</span>
        <button type="button" onClick={onClose} className="rounded-md p-1 text-slate-500 hover:bg-slate-100" aria-label="Zamknij menu">
          <X size={16} />
        </button>
      </div>
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-[280px] max-w-[88vw] transform bg-white transition-transform duration-200 lg:hidden ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        {nav}
      </div>
    </>
  )
