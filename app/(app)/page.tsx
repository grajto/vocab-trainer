...
export default async function HomePage() {
  ...
  // Streak calculation based on settings
  ...
  const jumpBackIn: Array<{ deckId: string; name: string; mode: string; sessionId: string; progress: string; startedAt: string; isActive: boolean }> = []
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-10">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/learn" prefetch={true} className="text-sm text-indigo-600 hover:text-indigo-700 font-medium">
            Nowa sesja →
          </Link>
        </div>
        {jumpBackIn.length === 0 ? (
      ...
      <section className="flex justify-center">
        <Link href="/library" prefetch={true} className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-indigo-600">
