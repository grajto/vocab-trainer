    ...
    <div className="min-h-screen" style={{ background: 'var(--surface-muted)' }}>
      <nav className="backdrop-blur-sm px-6 py-4" style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <Link href="/dashboard" className="text-lg font-semibold tracking-tight text-[var(--primary)]">Dashboard</Link>
        </div>
      </nav>
      <main className="max-w-lg mx-auto px-6 py-8 space-y-6">
        <h2 className="text-xl font-semibold text-[var(--text)]">Start Learning</h2>
        {decks.docs.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-sm mb-2 text-[var(--text-soft)]">No decks available.</p>
            <Link href="/decks" className="text-sm underline underline-offset-2 text-[var(--primary)]">Create a deck first</Link>
          </div>
        ) : (
          <StartSessionForm
